...
| Tweak Neovim                  | `neovim/nvf.nix` — every nvf option spelled out (derived from nvf's maximal sample); edit the option in place, no `mkForce` needed. See `docs/neovim-cheatsheet.md` |
| GNOME dconf / extensions      | `gnome/gnome.nix`, `gnome/gnome-extensions.nix` |

## CONVENTIONS (layer-specific)
- Add feature modules under their category and import them from its `default.nix`; keep `home.nix` limited to imports.
