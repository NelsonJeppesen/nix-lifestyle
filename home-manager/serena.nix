# serena.nix - Semantic code tools exposed to OpenCode over MCP
{
  config,
  lib,
  pkgs,
...
in
{
  ...
  # Reuse Nix-managed Ansible tools instead of letting Serena download its own
  # language server. Install a regular writable file because Serena records
  # registered projects in it; preserve that list across rebuilds while keeping
  # the rest declarative. Memory is disabled because OpenCode already has one.
