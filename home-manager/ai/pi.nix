...
let
  piMcpAdapter = pkgs.buildNpmPackage {
    ...
    version = "2.11.0";
    src = pi-mcp-adapter;
    # Upstream's lock file omits integrity on three nested dev-dependency entries.
    postPatch = ''
      ${lib.getExe pkgs.jq} '
...
in
{
  ...
  home.file = {
    ...
    # Mirror OpenCode's enabled MCP servers, plus its read-only Slack server requested for Pi testing.
    ".pi/agent/mcp.json".text = builtins.toJSON {
      ...
      mcpServers = {
        ...
        workiq = {
          inherit (config.programs.mcp.servers.workiq) command args;
          lifecycle = "lazy";
        };
        terraform = {
